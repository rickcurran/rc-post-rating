...
function rcpr_post_rating_shortcode( $atts ) {
    ...
    if ( $type == 'button' ) {
        $data .= '<button class="' . esc_attr( $classes ) . ' rating-up">' . esc_attr( $up ) . '</button>';
        $data .= '<button class="' . esc_attr( $classes ) . ' rating-down">' . esc_attr( $down ) . '</button>';
    } else {
        $data .= '<a href="#' . sanitize_title( esc_attr( $up ) ) . '" class="' . esc_attr( $classes ) . ' rating-up">' . esc_attr( $up ) . '</a>';
        $data .= '<a href="#' . sanitize_title( esc_attr( $down ) ) . '" class="' . esc_attr( $classes ) . ' rating-down">' . esc_attr( $down ) . '</a>';
    }
    $data .= '</div>';
